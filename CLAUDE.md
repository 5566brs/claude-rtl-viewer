...
- `POST /api/mode` (`{file, mode}`) → set the permission mode (`default` | `acceptEdits` | `plan` | `bypassPermissions`) for a conversation. Recorded in `pendingModes` for the next spawn and applied live (`Query.setPermissionMode`) if a process is running. Does **not** spawn a process on its own. Returns 409 with the SDK reason when a running session refuses the switch (notably `bypassPermissions`, which the CLI only allows when launched with it) — the client reverts the mode button so it never lies about the agent's real mode.
